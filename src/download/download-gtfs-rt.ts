...
export async function downloadGtfsRt(
	...
	await Promise.allSettled(
		realtimeFeedHrefs.map(async (realtimeFeedHref) => {
			...
			if (!response.ok)
				throw new Error(
					`Failed to download feed at '${realtimeFeedHref}' (status ${response.status}).`,
				);
			...
			for (const entity of entities) {
				if (entity.tripUpdate) {
					tripUpdates.push(
						typeof mapTripUpdate === "function"
							? mapTripUpdate(entity.tripUpdate)
							: entity.tripUpdate,
					);
				}
				...
				if (entity.vehicle) {
					vehiclePositions.push(
						typeof mapVehiclePosition === "function"
							? mapVehiclePosition(entity.vehicle)
							: entity.vehicle,
					);
				}
			}
