...
const getCalls = (
	...
	if (
		...
		atWithMargin.epochSeconds <
			(firstCall.expectedArrivalTime ?? firstCall.aimedArrivalTime).epochSeconds
	)
		return;
...
const getTripFromDescriptor = (gtfs: Gtfs, tripDescriptor: TripDescriptor) => {
	...
	if (typeof tripDescriptor.routeId !== "undefined" && trip.route.id !== tripDescriptor.routeId)
		return;
	if (
...
export async function computeVehicleJourneys(source: Source) {
	...
	try {
		...
		for (const vehiclePosition of vehiclePositions) {
			...
			if (typeof vehiclePosition.trip !== "undefined") {
				...
				if (typeof trip !== "undefined") {
					...
					if (
						now
							.since(Temporal.Instant.fromEpochSeconds(vehiclePosition.timestamp))
							.total("minutes") >= 10
					) {
						...
						if (
							Temporal.Instant.compare(
								now,
								(lastCall.expectedDepartureTime ?? lastCall.aimedDepartureTime).toInstant(),
							)
						) {
							continue;
						}
			...
			const tripRef =
				typeof journey !== "undefined"
					? (source.mapTripRef?.(journey.trip.id) ?? journey.trip.id)
					: undefined;
		...
		for (const journey of source.gtfs.journeys) {
			...
			if (typeof journey.trip.block !== "undefined" && handledBlockIds.has(journey.trip.block))
				continue;
			if (typeof source.allowScheduled === "function" && !source.allowScheduled(journey.trip))
				continue;
