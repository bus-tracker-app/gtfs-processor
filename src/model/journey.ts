...
export class Journey {
	...
	guessPosition(at: Temporal.Instant): JourneyPosition {
		...
		const nextArrivalTime = (nextCall.expectedArrivalTime ?? nextCall.aimedArrivalTime)
			.epochSeconds;
		...
		const estimatedDistanceTraveled =
			...
			(nextCallDistanceTraveled - monitoredCallDistanceTraveled) * percentTraveled;
		...
		return {
			latitude:
				currentShapePoint.latitude + (nextShapePoint.latitude - currentShapePoint.latitude) * ratio,
			longitude:
