...
if (typeof configurationPath !== "string") {
	...
	console.error(
		'Note: if no argument is given, environment variable "CONFIGURATION_PATH" will be used.',
	);
	process.exit(1);
}
...
const results = await Promise.allSettled(
	configuration.sources.map((source) => initLimitFn(() => loadGtfs(source))),
);
	...
	for (const source of configuration.sources) {
		...
		source.gtfs.journeys.sort((a, b) =>
			Temporal.ZonedDateTime.compare(
				a.calls.at(0)!.aimedArrivalTime,
				b.calls.at(0)!.aimedArrivalTime,
			),
		);
...
async function computeAndPublish() {
	...
	try {
		...
		const failures = computationResults
			.filter((result) => result.status === "rejected")
			.map((result) => result.reason);
		updateLog(
			"%s ► Publishing %d vehicle journey entries.",
			getTimestamp(),
			vehicleJourneys.length,
		);
		await Promise.all(
			vehicleJourneys.map((vehicleJourney) =>
				redis.publish(channel, JSON.stringify(vehicleJourney)),
			),
		);
		updateLog(
